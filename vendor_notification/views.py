...
class Calendar(calendar.HTMLCalendar):
    ...
    def formatday(self, day, weekday, events):
        ...
        for event in events_from_day:
            ...
            if self.custom_strftime("SU", event.start) == self.custom_strftime("SU", event.end):
                event_time = (
                    f"{self.custom_strftime('%H:%M', event.start)} - {self.custom_strftime('%H:%M', event.end)}"
                )
            else:
                event_time = (
                    f"{self.custom_strftime('SU %H:%M', event.start)} - {self.custom_strftime('SU %H:%M', event.end)}"
                )
        ...
        if day == 0:
            ...
            return '<td class="%s"><strong>%d</strong>%s</td>' % (
                css_class,
                day,
                events_html,
            )
    ...
    def formatmonth(self, theyear, themonth):
        events = models.Maintenance.objects.filter(Q(start__month=themonth) | Q(end__month=themonth)).annotate(
...
# MaintenanceSchedule
class MaintenanceScheduleView(View):
