...
def _netbox_circuitmaintenance(maintenance, mail):
    ...
    # Connect to Netbox
    nb = pynetbox.api(NB_URL, token=NB_API_TOKEN)
    nb.http_session.verify = False

    # Check if the maintenance event exists in Netbox
    nb_maintenance = nb.plugins.maintenance.circuitmaintenance.get(
    ...
    if nb_maintenance == None:
        ...
        if nb_provider != None:
            ...
            nb_maintenance = nb.plugins.maintenance.circuitmaintenance.create(
                ...
                start=datetime.utcfromtimestamp(int(maintenance["start"])).strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
                end=datetime.utcfromtimestamp(int(maintenance["end"])).strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
            )
            ...
            print(
                "Unable to match provider with one in Netbox. Parsed provider is: "
        ...
        print(
            "Maintenance event is existing. Updating details "
            + maintenance["maintenance_id"]
        )

        # Update existing maintenance event
        nb_maintenance.status = maintenance["status"]
        nb_maintenance.start = datetime.utcfromtimestamp(
            int(maintenance["start"])
        ).strftime("%Y-%m-%d %H:%M:%S")
        nb_maintenance.end = datetime.utcfromtimestamp(
            int(maintenance["end"])
        ).strftime("%Y-%m-%d %H:%M:%S")
        nb_maintenance.save()
    ...
    for circuit in maintenance["circuits"]:
        ...
        if nb_circuit != None:
            ...
            nb_impact = nb.plugins.maintenance.circuitmaintenanceimpact.get(
                circuitmaintenance=nb_maintenance.id, circuit=nb_circuit.id
            )
            ...
            if nb_impact != None:
                ...
                if nb_impact.impact == circuit["impact"]:
                    print(
                        "Circuit is tagged on this maintenance event already. Impact is "
                        + circuit["impact"]
                    )
                else:
                    # Carrier impact in this notification doen't match previously saved impact. Use this notification as source of truth
                    print(
                        "Circuit is tagged on this maintenance event already. Impact does not match the carrier notification. Updating impact to "
                        + circuit["impact"]
                    )
            ...
            print(
                "Circuit ID "
                + circuit["circuit_id"]
                + " does not exist in Netbox. Unable to associate with maintenance event."
            )
    ...
    nb.plugins.maintenance.circuitmaintenancenotifications.create(
        ...
        email_recieved=datetime.utcfromtimestamp(int(maintenance["stamp"])).strftime(
            "%Y-%m-%d %H:%M:%S"
        ),
    )
...
def lambda_handler(event, context):
    ...
    try:
        ...
        file_content = s3_client.get_object(
            Bucket=S3_BUCKET_NAME, Key=event["Records"][0]["ses"]["mail"]["messageId"]
        )["Body"].read()
    except:
    ...
    try:
        ...
        mail = email.message_from_string(
            file_content.decode("utf-8"), policy=policy.default
        )
    except:
